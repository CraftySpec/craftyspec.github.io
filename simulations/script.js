// Diatomic vibration–rotation spectrum simulator
...
let vibFreq = parseFloat($('vibFreq').value)   // band origin
...
function calculateSpectrum() {
  ...
  for (let J = 0; J < maxJ; J++) {
    ...
    const R_intensity = (J + 1) * population

    result.push({
      branch: 'R',
      J: J,
      wavenumber: R_wavenumber,
    ...
    if (J > 0) {
      const P_wavenumber =
        nu0 -
        (2 * p.B * J) -
        (4 * p.D_user * Math.pow(J, 3))

      const P_intensity = J * population

      result.push({
        branch: 'P',
        J: J,
        wavenumber: P_wavenumber,
...
function updateSimulation() {
  ...
  const visible = lines.filter(l => Number.isFinite(l.wavenumber))
...
function drawAtom(x, y, radius, label) {
  ...
  const labelOffset = radius + 22;   // pushes text below the atom
